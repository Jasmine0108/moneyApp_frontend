...
export default function checkSumScreen() {
  ...
  useEffect(() => {
    ...
    if (isFocused) getGroupMember()
    getTotalMoney()
  }, [isFocused])

  const getTotalMoney = async () => {
    var _amount = await AsyncStorage.getItem('@snapshotAmountModify')
    setTotal(_amount)
  }

  const storeMoney = async () => {
    const filteredResponse = showData.filter((data) => {
      if (data.userId != 'init') return data
    })
    ...
    AsyncStorage.setItem('@check_sumResponseModify', JSON.stringify(response))
    AsyncStorage.setItem('@snapshotAmountModify', total)
  }

  const handleCancelButton = () => {
    router.navigate('/modify_bill')
  }
  const handleConfirmButton = () => {
    ...
    for (let i = 0; i < showData.length; ++i) {
      ...
      if (data.amount == '' && data.userId != 'init') {
        ...
        isProblem = true
        break
      } else if (data.amount == '' && data.userId == 'init') count_empty += 1
    ...
    if (isProblem == false) {
      ...
      setAddSum(sum)
      setSumOpacity(1)
      if (sum == parseInt(total)) setConfirmButtonText('儲存')
    }
  }
  const handleAddButton = () => {
    if (count < memberInfo.length) {
      ...
      setShowData(tmpData)
      setCount(count + 1)
      //('addButtonData', showData)
    } else Alert.alert('Boxes exceed group member number.')
  }

  const handleOnSelectFinished = (
    selectRowId: string,
    selectUserId: string
  ) => {
    setSumOpacity(0)
  ...
  const handleAbleSelectMember = (value: string) => {
    ...
    if (pre_now_select[1] != 'init') {
      filteredMembers = ableSelectMember.filter(function (member) {
        if (member.userId != pre_now_select[1]) return member.userId
      })
    }
    if (pre_now_select[0] != 'init') {
      let user = memberInfo.find(
        (member) => member.userId === pre_now_select[0]
      )
...
export function UserAddRow(prop: prop) {
  ...
  const calculate_height = () => {
    if (prop.ableSelectmember.length * 45 > 300) setMaxHeight(300)
  ...
  const setUserSelection = (newSelectMemberId: string) => {
    ...
    let user = prop.allMembers.find(
      (member) => member.userId === newSelectMemberId
    )
      ...
      <XStack>
        ...
        <Select value={selectMemberId} onValueChange={setUserSelection}>
          <Select.Trigger
            width="45%"
            px="1%"
            iconAfter={<AntDesign name="caretdown" size={15} color="#BCBCBC" />}
            {...(parseInt(prop.id) % 2 == 0
              ? { bg: '#FFFFFF' }
              : { bg: '#DDDDDD' })}
          >
